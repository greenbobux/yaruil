...
import { elementLike } from "./util/types";
export type JsxNode = ReadonlyArray<elementLike> | ReadonlyMap<string | number, elementLike> | elementLike;
declare global {
	namespace JSX {
		type Element = elementLike;
		export type JsxAttributes<T extends Instance> = {
			[P in WritablePropertyNames<T>]?: T[P] | (<V extends []>(...args: V) => T[P] | undefined);
		} & {
			[K in InstanceEventNames<T> as K extends string ? `on:${K}` : never]?: T[K] extends RBXScriptSignal<infer C>
